if (GabeDFPU === undefined) var GabeDFPU = {};
if (typeof CCSE == 'undefined') Game.LoadMod('https://klattmose.github.io/CookieClicker/' + (0 ? 'Beta/' : '') + 'CCSE.js');
GabeDFPU.name = 'GabeDFPU';
GabeDFPU.version = '1';
GabeDFPU.GameVersion = '2.053';

GabeDFPU.launch = function () {

	GabeDFPU.init = function(){
		...
		*/
		...
		//chances for 91 & 92 have been doubled, 1/5 and 1/4, because a 1/10 or 1/8 chance *per reincarnation* to get a slightly worse or slightly better egg, to me, feels incredibly stupid, especially for a Heavenly Upgrade of all things
		...
		Cost: Unknown.
		Parent: Unknown.
		//renamed to be Heavenly & Wrathful respectively.
		//combined into one HU, but two boosters.
		//:D

		below are from dpareja
		...
		*/
		...
		- Unlocks from 29. //kinda scrapped
		...
		//ehhh, we keep em, but make them only show up if you have Perfect Cookie Selector the mod.
		...
		//72 and 73 will be removed due to them already existing. (ehh, we can add more)
		...
		//below two are scrapped due to, kinda being hard-coded (didn't give it a shot, but given what I've seen, choice appears to be hard-coded a bit, or at least ruining modding compatability)
		...
		118. (by Freezepond) //unfortunately scrapped
		Name: Humble Double
		...
		//^ will only do once I find credit

		//below three are scrapped due to there being too many season stuff already, it is a loss of flavor, and season checks are Game.season == 'seasonName' so multiple seasons wouldn't really work :(
		...
		*/
		...
		MOD.cookclickval = 1;
		MOD.autoclickerclickspersecond = 15;

		MOD.ApplyOrder = function(upgrade){
			//apparently adding upgrades within order of other things, everything fuckin breaks...
			//don't blame me! I just found the comment:
			//WARNING : do NOT add new upgrades in between, this breaks the saves. Add them at the end !
			//and didn't wanna test it!!!
			//used to take the order of upgrade to allow them to be in-between other upgrades, but that, like the one comment I found suggests, might break everything.
		...
		Game.mods["GabeDFPU"].evilFace = "We're having this here just so that it's \"Global\" within the mod's scope, so that some code can work"
		MOD.EvilFaceAAA = function(){
			if (Game.Has('Evil face') && (Game.mods["GabeDFPU"].TooUglyTimesBought <= 0 || Game.mods["GabeDFPU"].TooUglyTimesBoughtTOTAL <= 0)) {
				Game.mods["GabeDFPU"].evilFace = document.createElement('div');
				Game.mods["GabeDFPU"].evilFace.className = 'EvilFace';
				Game.mods["GabeDFPU"].evilFace.style.backgroundImage = `url('${Game.mods["GabeDFPU"].dir}/imperfectCookie.png')`;
				Game.mods["GabeDFPU"].evilFace.style.position = 'absolute';
				Game.mods["GabeDFPU"].evilFace.style.width = '256px'
				Game.mods["GabeDFPU"].evilFace.style.height = '256px'
				Game.mods["GabeDFPU"].evilFace.style.top = '-128px'
				Game.mods["GabeDFPU"].evilFace.style.left = '-128px'
				Game.mods["GabeDFPU"].evilFace.style.zIndex = 10001;
				Game.mods["GabeDFPU"].evilFace.style.pointerEvents = 'none'; //dont do 'block', we need clicking
				Game.mods["GabeDFPU"].evilFace.style.backgroundSize = 'contain';
				Game.mods["GabeDFPU"].evilFace.style.backgroundRepeat = 'no-repeat';
				if (Game.mods["CookieMonster"]) {
					document.children[0].children[1].children[0].children[1].children[17].children[8].appendChild(Game.mods["GabeDFPU"].evilFace);
				} else {
					document.children[0].children[1].children[0].children[1].children[16].children[8].appendChild(Game.mods["GabeDFPU"].evilFace);
				}
				}
		}
		...
		//guess we puttin all our variables and specific functions up here!
		MOD.jadeSlotDuped = false;
		MOD.jadeSlotID = Infinity;
		MOD.jadeSlotted = -1;

		MOD.allSlotDuped = false;
		MOD.allSlotJadeSlotDuped = false;
		MOD.allSlot = {
			"jade": -1,
			"ruby": -1,
			"diamond": -1,
		...
		MOD.loadAltarSlots = function(){
			if (CE.PantheonInjected == false) return;

			if (Game.Has('Additional altar') && MOD.jadeSlotDuped == false) {
				MOD.jadeSlotDuped = true;
				MOD.jadeSlotID = CE.addSlot("jade");
				Game.Objects["Temple"].minigame.slot[MOD.jadeSlotID] = MOD.jadeSlotted;
				// CCSE.RedrawGods();
			}

			if (Game.Has("Ankh") && MOD.allSlotDuped == false) {
				MOD.allSlotDuped = true;
				
				MOD.allSlotID["diamond"] = CE.addSlot("diamond");
				MOD.allSlotID["ruby"] = CE.addSlot("ruby");
				MOD.allSlotID["jade"] = CE.addSlot("jade");

				Game.Objects["Temple"].minigame.slot[MOD.allSlotID["jade"]] = MOD.allSlot["jade"];
				Game.Objects["Temple"].minigame.slot[MOD.allSlotID["ruby"]] = MOD.allSlot["ruby"];
				Game.Objects["Temple"].minigame.slot[MOD.allSlotID["diamond"]] = MOD.allSlot["diamond"];
				// CCSE.RedrawGods();
			}

			if (Game.Has("Ankh") && Game.Has('Additional altar') && MOD.allSlotJadeSlotDuped == false) {
				MOD.allSlotJadeSlotDuped = true;
				MOD.allSlotID["jade2"] = CE.addSlot("jade");
				Game.Objects["Temple"].minigame.slot[MOD.allSlotID["jade2"]] = MOD.allSlot["jade2"];
				// CCSE.RedrawGods();
			}
			CCSE.RedrawGods();
		}

		MOD.pantheonChecked = false;
		MOD.grimoireChecked = false;

		MOD.MiniGameChecks = function(){
			if (CE.PantheonInjected && Game.mods["GabeDFPU"].pantheonChecked == false) {
				Game.mods["GabeDFPU"].pantheonChecked = true;

				Game.customPantheonSwapT.push(function(time){
					var tmod = 1;
					if (Game.Has("Second priest")) tmod/=2;
					if (Game.Has("Ankh")) tmod/=2;
					return (time * tmod);
				})
			}

			if (CE.GrimoireInjected && Game.mods["GabeDFPU"].grimoireChecked == false) {
				Game.mods["GabeDFPU"].grimoireChecked = true;

				Game.customSpellCostMod.push(function(out,spell){
					var costmod = 0;
					if (Game.Has("Efficient casting")) costmod-=2;
					
					var finalcost = (out+costmod);
					if (Game.Has("Ankh")) finalcost/=2;
					return finalcost;
				});

				Game.customMagicPS.push(function(magicPS,magic){
					var mpsMult = 1;
					if (Game.Has("Supercharged recharge")) mpsMult*=2;
					if (Game.Has("Ankh")) mpsMult*=2;

					var buffMult = 1;
					for (let i in Game.buffs) {
						if (Game.buffs.multCpS) {
							buffMult*=Game.buffs.multCpS
						}
					}
					if (Game.Has("Magical cookies")) {
						mpsMult*=buffMult;
					}

					return (magicPS * mpsMult);
				})

				Game.customFailChance.push(function(failChance,spell){
					var failChanceMult = 1;
					if (Game.Has("Increased aptitude")) failChanceMult/=3;
					if (Game.Has("Ankh")) failChanceMult/=2;

					return (failChance*failChanceMult);
				})
			}
		}
		...
		CCSE.NewAchievement("Officially bought","Buy \"Too ugly!\" <b>6666666666666666</b> times.<q>It doesn't get uglier than this.</q>",[21,6]);
		Game.last.pool = 'shadow';
		MOD.ApplyAchievOrder();
		...
		//order manip (sowwy)
		// Game.Upgrades["Halo gloves"].order +=0.001;
		// Game.Upgrades["Lucifer"].order -=0.0009;
		// Game.Upgrades["Divine discount"].order += 0.01;
		// Game.Upgrades["Divine sales"].order += 0.01;
		// Game.Upgrades["Divine bakeries"].order += 0.01;
		// Game.Upgrades["Lucky digit"].order += 0.0001;
		// Game.Upgrades["Lucky payout"].order = Game.Upgrades["Divine bakeries"].order;
		...
		MOD.ApplyOrder("Synergies Vol. III");
		...
		MOD.ApplyOrder("Night gaming deluxe");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Perfect idling 2");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Deep sleep production");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Passive perfection");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Distilled essence of redoubled luck");
		...
		MOD.ApplyOrder("Box of pastries");
		...
		MOD.ApplyOrder("Starter kitchen");
		...
		MOD.ApplyOrder("Starter sprouts");
		...
		MOD.ApplyOrder("Starter chemistry");
		MOD.applyCredit("1234abcdcba4321")

		CCSE.NewHeavenlyUpgrade("Heavenly grandmas","Grandmas are <b>twice as efficient</b>.<q>A heavenly grandma to permanently assist you in your quest of baking cookies.</q>",100000000000000,[10,3,MOD.custIcons],-402,-1002,["Starter light"])
		MOD.ApplyOrder("Starter light");
		MOD.applyCredit("1234abcdcba4321 / thecrazyhyperbanana")

		CCSE.NewHeavenlyUpgrade("Stacked clicking","Clicking is <b>0.1% more powerful</b> for every cursor you have.<q>Through rather simple \"magic\" of making cursors independently stack onto your own cursor, each cursor effectively repays itself in little to no time!<br>Until inflation hits.</q>",15000000,[11,3,MOD.custIcons],-496,-618,["Starter kitchen"])
		MOD.ApplyOrder("Heavenly grandmas"); //not *forcefully balanced to the point of effectively changing it entirely*, but a small division of 10, from 1% more powerful, to 0.1% more powerful;s
		MOD.applyCredit("thecrazyhyperbanana")

		CCSE.NewHeavenlyUpgrade("Repetitive clicking","Clicking is <b>0.1% more powerful</b> for every cursor you have. <small>(multiplicative)</small><q>If we just keep stacking more and more, surely nothing bad could happen!</q>",1500000000,[12,3,MOD.custIcons],-396,-721,["Stacked clicking"]);
		MOD.ApplyOrder("Stacked clicking"); //Same as above
		MOD.applyCredit("1234abcdcba4321")
		...
		MOD.ApplyOrder("Persistent memory");
		...
		MOD.ApplyOrder("Lucy in the sky with cookies");
		MOD.applyCredit("Iceklaus");
		...
		MOD.ApplyOrder("Five-finger discount");
		MOD.applyCredit("jupiterboy");
		CCSE.NewHeavenlyUpgrade('Autoclick upgrade', "Unlocks an autoclicker that autoclicks the cookie <b>10 times</b> per second when toggled.<q>As if a third helping hand from heaven was granted</q>",5000000000,[0,6],-12,-444,["Halo gloves"])
		Game.last.descFunc = function(){return `Unlocks an autoclicker that autoclicks the cookie <b>${Game.mods["GabeDFPU"].autoclickerclickspersecond} times</b> per second when toggled.<q>As if a third helping hand from heaven was granted</q>`}
		MOD.ApplyOrder("Cursor black magic");
		MOD.applyCredit("narcis233Cookie");

		//making it a toggle, it's just meant to be a click every half-a-second, or two clicks every second...
		//but I guess a registered logic hook somehow causes some other autoclicker mod to break.
		//don't ask me, Deinonycorvus said it breaks auto-clicker mods (none stated to break (functions pushed to hooks do not overwrite other functions in said hook, combined with it being a simple if-statement into Game.ClickCooke(), I don't understand it, I have no evidence of it breaking, but alright!))
		//also making it a toggle would just help in general, and a small buff to try to put it on par with normal clicking rates while idling or while active (but not to an extent of a click every milisecond)
		CCSE.NewUpgrade("Autoclicker [off]","Turning this on will autoclick the cookie <b>10 times</b> per second. Has no cost.",0,[7,8,MOD.custIcons]);
		Game.last.descFunc = function(){return `Turning this on will autoclick the cookie <b>${Game.mods["GabeDFPU"].autoclickerclickspersecond} times</b> per second. Has no cost.`}
		Game.last.toggleInto='Autoclicker [on]'
		Game.last.pool = 'toggle'
		MOD.ApplyOrder();
		CCSE.NewUpgrade("Autoclicker [on]","Currently autoclicking the cookie <b>10 times</b> per second. Has no cost.",0,[0,6]);
		Game.last.descFunc = function(){return `Currently autoclicking the cookie <b>${Game.mods["GabeDFPU"].autoclickerclickspersecond} times</b> per second. Has no cost.`}
		Game.last.toggleInto='Autoclicker [off]'
		Game.last.pool = 'toggle'
		MOD.ApplyOrder();
		...
		//MORE EGGS (Chocolate);
		CCSE.NewHeavenlyUpgrade("Rich chocolate","Chocolate egg bursts into <b>7.5%</b> of your current bank, or <b>600x CpS</b>, whichever is higher.<q>A deliciously rich chocolate egg to make you rich!</q>",99999999,[18,12],-1980,-56,["Box of gifts"]);
		MOD.ApplyOrder();
		MOD.applyCredit("The_Cookie_God");
		CCSE.NewHeavenlyUpgrade("Richer chocolate","Chocolate egg bursts into <b>10%</b> of your current bank, or <b>1200x CpS</b>, whichever is higher.<q>The richer the chocolate, the richer you'll become</q>",9999999999,[7,6,MOD.custIcons],-2068,-116,["Rich chocolate"]);
		MOD.ApplyOrder();
		MOD.applyCredit("1234abcdcba4321");
		CCSE.NewHeavenlyUpgrade("Richest chocolate","Chocolate egg bursts into <b>12.5%</b> of your current bank, or <b>1800x CpS</b>, whichever is higher.<q>How can an egg contain oh so many cookies?</q>",99999999999,[8,6,MOD.custIcons],-2152,-176,["Richer chocolate"]);
		MOD.ApplyOrder();
		MOD.applyCredit("1234abcdcba4321");
		CCSE.NewHeavenlyUpgrade("Dual chocolate","Adds a bonus chocolate egg, functioning the same but awards <b>half as much</b>.<br>Small chance to be unlocked when <b>Chocolate egg</b> is unlocked.<q>Rather rare and elusive, almost as elusive as that sugar bunny!</q>",9999999999,[9,6,MOD.custIcons],-2076,4,["Rich chocolate"]);
		MOD.ApplyOrder();
		MOD.applyCredit("1234abcdcba4321");
		CCSE.NewHeavenlyUpgrade("Strengthened twin","Bonus chocolate egg awards <b>twice as much</b>, and is slightly more common.<q>A bit more worthful, but good luck even finding it!</q>",9999999999999,[10,6,MOD.custIcons],-2152,88,["Dual chocolate"]);
		MOD.ApplyOrder();
		MOD.applyCredit("1234abcdcba4321");
		CCSE.NewUpgrade("Bonus chocolate egg","Contains <b>a lot of cookies</b>.<br>Cost scales with how many eggs you own.<br>Does not count as an egg.<q>A rather elusive and secret egg, very few ever find it.<br>It's like the chocolate egg, but flipped horizontally and with slightly different colors.</q>",999,[9,6,MOD.custIcons])
		Game.last.priceFunc=function(){return Math.pow(3,Game.GetHowManyEggs())*999;}
		Game.last.buyFunction=function()
		{
			...
			if (!Game.Has("Strengthened twin")) {
				cookies/=2;
			}
			Game.Notify('Bonus chocolate egg',loc("The egg bursts into <b>%1</b> cookies!",Beautify(cookies)),Game.Upgrades['Bonus chocolate egg'].icon);
			Game.Earn(cookies);
		
			// Game.Upgrades['Bonus chocolate egg'].buyFunction injection point 0
		...
		Game.last.order = Game.Upgrades["Chocolate egg"].order + 0.00001;
		...
		*/
		...
		MOD.ApplyOrder("Even longer seasons");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Ever longer seasons");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Seasonal saving");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Season automation");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Season switcher");
		...
		MOD.ApplyOrder("Autoclick upgrade");
		MOD.applyCredit("Freezepond");
		...
		MOD.ApplyOrder("Shimmering power");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		//todo, figure out what the hell is going on with the color of the code here (doesn't matter, just is odd)
		CCSE.NewHeavenlyUpgrade('Redoubled essence of redoubled essence of redoubled luck',"Golden cookies have a <b>7%</b> chance to have <b>doubled effects</b>.<br>Only active after clicking <b>77 golden cookies</b> in the current ascension.<br><small>(stacks)</small><q>I feel as if we've done enough \"redoubling\" here.<br>That and you just have three empty vials that all look the same.</q>",777777777,[27,12],-661,157,["Redoubled essence of redoubled luck"])
		MOD.ApplyOrder("Redoubled essence of redoubled luck");
		MOD.applyCredit("1234abcdcba4321");
		MOD.New();
		...
		MOD.ApplyOrder("Redoubled essence of redoubled essence of redoubled luck");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Endlessly shimmering");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Eternal presence");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Wrath gift box");
		Game.last.order += 0.001;
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Golden gift box");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Longer frenzy I");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Longer frenzy II");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Better frenzy I");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Longer elder frenzy I");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Longer elder frenzy II");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Better elder frenzy I");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Better frenzy II");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Longer click frenzy I");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Longer click frenzy II");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Better click frenzy I");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Better click frenzy II");
		MOD.applyCredit("1234abcdcba4321");
		Game.goldenCookieUpgrades.push(Game.last.name);
		...
		MOD.ApplyOrder("Wrath gift box");
		Game.last.order -=0.0001;
		Game.last.order +=0.00001;
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.buyLoop = async function(calcToDo) {
			for (let i = 0; i < calcToDo; i++) {
				...
				if (MOD.TooUglyTimesBought < 6666666666666666) {
					Game.Upgrades["Too ugly!"].bought = 0;
				} else {
					Game.Win("Officially bought");
					MOD.TooUglyUpdate();
					Game.Upgrades["Too ugly!"].bought = 1;
					return;
				}
		...
		Game.last.priceFunc = function () {
			//I am so sorry
			var price = Game.Upgrades["Too ugly!"].basePrice + (Game.mods["GabeDFPU"].TooUglyTimesBoughtTOTAL * 666666);
			price = Math.pow(price,1 + Math.log10(0.001 * (Game.mods["GabeDFPU"].TooUglyTimesBoughtTOTAL + 1)));
			return price;
		...
		Game.last.descFunc = function() {
			...
			const milestones = [ //milestones as Too ugly! gives specific effects depending on how many times you've purchased it
				...
				{count: 6666, effect: "+6666% CpS multiplier <small>(additive)</small>"},
			...
			if (nextMilestone == "<b>All effects unlocked!</b>" || nextMilestone == `+6 shadow achievements <small><small>(I removed two things in this effect and the last due to balance... so sorry!)</small></small>`) {
				return `
		...
		MOD.ApplyOrder("Sacrilegious corruption");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Ungodly mutations");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("First-day shipping");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Divine bakeries");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Super divine discount");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Super divine sales");
		MOD.applyCredit("1234abcdcba4321");
		...
		CE.NewTag("Forcefully Balanced (not 1:1)", "#FF0000", false, ["Super divine sales","Super divine bakeries","Super divine discount","Too ugly!"]);
		...
		MOD.ApplyOrder("Wrinkly cookies");
		MOD.applyCredit("1234abcdcba4321");
		...
		MOD.ApplyOrder("Ancient gauntlets");
		MOD.applyCredit("1234abcdcba4321");
		...
		if (!cookieMonsterDetected) {
			...
			MOD.ApplyOrder("Unshackled You"); //note, 1234abcdcba4321 did NOT make the building itself, just said a better prism, but now I'm making a better You...
			MOD.applyCredit("1234abcdcba4321");
			...
			Game.Notify("Uh oh!","Due to Cookie Monster, quite a few functions are overwritten entirely, and the mod's code is not organized (it's unreadable, at least for my eyes).",[],15);
		}

		MOD.Singularitited = false;
		MOD.SingularititeCheck = function(shouldNotif){
			if (cookieMonsterDetected) {return} //it doesn't work, Cookie Monster's code is unreadable, and overwrites many functions entirely... sowwy!
			...
			if (Game.Has("Unbounded")&& Game.mods["GabeDFPU"].Singularitited == false) {
				...
				MOD.ApplyOrder("Unshackled glimmeringue");
				...
				//I'm losing steam despite the ULTRAKILL "HEAVENLY LIGHTS //FLICKERING// INTO THE AFTERLIFE" music.
				//I *have* lost steam.
				...
				Game.last.dname = "Cookie clicker "+(Math.round(Game.Objects["Idleverse"].amount / 70)+Game.mods["GabeDFPU"].cookclickval).toString();
				Game.last.descFunc=function(){
				...
				Game.UpgradesByPool.kitten.push(Game.last)
		...
		MOD.ApplyOrder("Instant research");
		MOD.applyCredit("1234abcdcba4321")
		...
		MOD.ApplyOrder("Lower inflation");
		MOD.applyCredit("1234abcdcba4321")
		...
		CCSE.NewHeavenlyUpgrade("Angelic mouse","You get more cookies per click the <b>faster you click</b>.<q>walk<br>run<br>stride<br>gallop</q>",111111,[7,9,MOD.custIcons],-359,-311,["Starter kit"])
		MOD.ApplyOrder("Halo cursor");
		//Game.last.order = Game.Upgrades["Halo cursor"].order - 0.00001;
		MOD.applyCredit("jupiterboy");
		MOD.New();

		CCSE.NewHeavenlyUpgrade("Divine baking I","Cookies are <b>25%</b> more powerful.<q>Seems like improving the workforce up here has a great-deal of effects down below!</q>",1000,[12,5,MOD.custIcons],-488,-74,["Heralds"])
		MOD.ApplyOrder("Heralds");
		MOD.applyCredit("Freezepond");
		MOD.New();
		CCSE.NewHeavenlyUpgrade("Divine baking II","Cookies are <b>25%</b> more powerful.<q>Firing those old bakers and replacing them with your own ensures they are designed specifically to your tastes!</q>",10000,[13,5,MOD.custIcons],-556,-129,["Divine baking I"])
		MOD.ApplyOrder("Divine baking I");
		MOD.applyCredit("Freezepond");
		MOD.New();
		CCSE.NewHeavenlyUpgrade("Divine baking III","Cookies are <b>25%</b> more powerful.<q>Revamping some of the tools they used to use to be easier to use, and require less bakers for the same amount of labor (although keeping the same number of bakers around means you get more cookies).</q>",100000,[14,5,MOD.custIcons],-607,-200,["Divine baking II"])
		MOD.ApplyOrder("Divine baking II");
		MOD.applyCredit("Freezepond");
		MOD.New();
		CCSE.NewHeavenlyUpgrade("Divine baking IV","Cookies are <b>25%</b> more powerful.<q>Shutting down the bakeries in place of factories much like your own in the mortal world simply for production, and specifically for you!</q>",1000000,[15,5,MOD.custIcons],-635,-281,["Divine baking III"])
		MOD.ApplyOrder("Divine baking III");
		MOD.applyCredit("Freezepond");
		MOD.New();

		CCSE.NewHeavenlyUpgrade("The universe...","Factories are <b>+4% more efficient</b>.",4444,[31,2],-354,-392,["Starter kit"]);
		MOD.ApplyOrder("Repetitive clicking");
		MOD.applyCredit("1234abcdcba4321");
		MOD.New();
		CCSE.NewHeavenlyUpgrade("...said 4.","Cookie production multiplier <b>+4%</b>.<br>Factories are <b>+44% more efficient</b>.",44444444,[30,2],-443,-395,["The universe..."]);
		MOD.ApplyOrder("The universe...");
		MOD.applyCredit("1234abcdcba4321");
		MOD.New();
		CCSE.NewHeavenlyUpgrade("Team force bread","Cookie production multiplier <b>+44%</b>.<br>Factories are <b>+444% more efficient</b>.<br>Factories are <b>4% cheaper</b>.",39002837000000,[4,21],-527,-380,["...said 4."]);
		MOD.ApplyOrder("...said 4.");
		MOD.applyCredit("1234abcdcba4321");
		MOD.New();

		CCSE.NewHeavenlyUpgrade("Golden power","Unlocks the <b>golden power</b>, which gives up <b>10%</b> of your CpS, in exchange for golden cookie buffs being <b>twice as powerful</b>.<q>Less idling, more clicking",9999,[11,9,MOD.custIcons],-296,378,["Golden switch"]);
		MOD.ApplyOrder("Golden idling");
		MOD.applyCredit("RedCookie6666");
		MOD.New();
		CCSE.NewUpgrade("Golden power [off]","Turning this on will make golden cookie <small>(and wrath cookie)</small> buffs <b>twice as powerful</b>, but reduces CpS by <b>10%</b>.<br>Cost is equal to 1 hour of production.",0,[10,9,MOD.custIcons]);
		Game.last.toggleInto='Golden power [on]'
		Game.last.priceFunc = function(){return Game.cookiesPs*60*60}
		Game.last.pool = 'toggle'
		MOD.ApplyOrder();
		CCSE.NewUpgrade("Golden power [on]","Golden power is currently making golden cookie <small>(and wrath cookie)</small> buffs <b>twice as powerful</b>; it also reduces CpS by <b>10%</b>.<br>Turning this off will revert those effects.<br>Cost is equal to 1 hour of production.",0,[11,9,MOD.custIcons]);
		Game.last.toggleInto='Golden power [off]'
		Game.last.priceFunc = function(){return Game.cookiesPs*60*60}
		Game.last.pool = 'toggle'
		MOD.ApplyOrder();
		
		//ALRIGHT LET'S FORCE MINESELF TO ADD A TON OF HUs!!!

		CCSE.NewHeavenlyUpgrade("Building boosters","Unlocks two building boosters.<br><b>Heavenly</b> and <b>Wrathful</b>.<q>Get rid of those pests, or praise the pocalypse.<br>Those are the choices.</q>",777666777666777,[14,9,MOD.custIcons],-334,-1076,["Heavenly grandmas"]);
		MOD.ApplyOrder("Heavenly grandmas");
		MOD.applyCredit("NeoSpearBlade");
		MOD.New();
		CCSE.NewUpgrade("Heavenly building booster [off]","Cookie production multiplier <b>+0.5%</b> for every building owned.<br>Percentage increases by <b>0.1%</b> for every 100 buildings owned.<br>Percentage decreases by <b>0.05%</b> for every wrinkler present.<br>Cost is equal to 1 hour of production.",0,[12,8,MOD.custIcons])
		Game.last.toggleInto='Heavenly building booster [on]'
		Game.last.priceFunc = function(){return Game.cookiesPs*60*60}
		Game.last.pool = 'toggle'
		Game.last.buyFunction = function(){Game.Upgrades["Wrathful building booster [off]"].unlocked = 0; Game.upgradesToRebuild=1;}
		MOD.ApplyOrder();
		CCSE.NewUpgrade("Wrathful building booster [off]","Cookie production multiplier <b>+0.5%</b> for every building owned during the Grandmapocalypse.<br>Percentage is based on the Grandmatriarch's status.<small>(Awoken: 0.5%. Displeased: 1.5%. Angered: 2.5%.)</small>.<br>Cost is equal to 1 hour of production.",0,[13,8,MOD.custIcons])
		Game.last.toggleInto='Wrathful building booster [on]'
		Game.last.priceFunc = function(){return Game.cookiesPs*60*60}
		Game.last.pool = 'toggle'
		Game.last.buyFunction = function(){Game.Upgrades["Heavenly building booster [off]"].unlocked = 0; Game.upgradesToRebuild=1;}
		MOD.ApplyOrder();
		CCSE.NewUpgrade("Heavenly building booster [on]","Currently giving cookie production multiplier <b>+0.5%</b> for every building owned.<br>Percentage increases by <b>0.1%</b> for every 100 buildings owned.<br>Percentage decreases by <b>0.05%</b> for every wrinkler present.<br>Turning it off will revert those effects.<br>Cost is equal to 1 hour of production.",0,[12,9,MOD.custIcons]);
		Game.last.toggleInto='Heavenly building booster [off]'
		Game.last.priceFunc = function(){return Game.cookiesPs*60*60}
		Game.last.pool = 'toggle'
		Game.last.buyFunction = function(){Game.Upgrades["Wrathful building booster [off]"].unlock();}
		MOD.ApplyOrder();
		CCSE.NewUpgrade("Wrathful building booster [on]","Currently giving cookie production multiplier <b>+0.5%</b> for every building owned during the Grandmapocalypse.<br>Percentage is based on the Grandmatriarch's status. <small>(Awoken: 0.5%. Displeased: 1.5%. Angered: 2.5%.)</small>.<br>Turning it off will revert those effects.<br>Cost is equal to 1 hour of production.",0,[13,9,MOD.custIcons]);
		Game.last.toggleInto='Wrathful building booster [off]'
		Game.last.priceFunc = function(){return Game.cookiesPs*60*60}
		Game.last.pool = 'toggle'
		Game.last.buyFunction = function(){Game.Upgrades["Heavenly building booster [off]"].unlock();}
		MOD.ApplyOrder();

		//more injecting yay (pushed 5 upgrades to either scrapped or for later if the public (community) demands it)

		//Game.Notify('Test','Test: <b><span style=color:#7af;>Diamond</span> <span style=color:#f30;>Ruby</span> <span style=color:#3f0;>Jade</span></b>',[23,15],2555);

		CCSE.NewHeavenlyUpgrade("Additional altar","Grants a second <b><span style=color:#3f0;>Jade</span> slot</b>, although no additional pantheon swaps.<q>Praise more lords!</q>",33333333,[18,7,MOD.custIcons],413,290,["Permanent upgrade slot III"]);
		MOD.ApplyOrder(); //yup, added an entire new pantheon slot, even added CE functions for it... the hell that is
		MOD.applyCredit("dpareja");
		MOD.New();
		Game.last.showIf = function(){
			return Game.Objects["Temple"].level >= 1;
		}

		CCSE.NewHeavenlyUpgrade("Second priest","Halves refill time for <b>pantheon swaps</b>.<q>About time you got a second priest!</q>",1555555,[17,8,MOD.custIcons],449,-1055,["God"]);
		MOD.ApplyOrder();
		MOD.applyCredit("dpareja");
		MOD.New();
		Game.last.showIf = function(){
		...
		CCSE.NewHeavenlyUpgrade("Efficient casting","Spells cost <b>2 less magic</b>.<q>Replacing the wand you've been using for something built for these spells turns out to actually be helpful.</q>",555555,[17,17],-841,-780,["Unholy bait"]);
		MOD.ApplyOrder();
		MOD.applyCredit("dpareja");
		MOD.New();
		Game.last.showIf = function(){
			return Game.Objects["Wizard tower"].level >= 1;
		}

		CCSE.NewHeavenlyUpgrade("Supercharged recharge","Magic regens <b>twice as fast</b>.<q>Charged supercharged recharged charge!</q>",10000000,[17,9,MOD.custIcons],-643,-370,["Divine baking IV"])
		MOD.ApplyOrder();
		MOD.applyCredit("dpareja");
		MOD.New();
		Game.last.showIf = function(){
		...
		CCSE.NewHeavenlyUpgrade("Magical cookies","Magic regen is now affected by <b>CpS buffs</b>. <small>(eg. Frenzy multiplies magic per second by 7)</small><q>These cookies have a taste of \"magic\".<br>It is not advised to eat these cookies.</q>",789789789,[28,12],-700,658,["Auric patience"]);
		MOD.ApplyOrder();
		MOD.applyCredit("dpareja");
		MOD.New();
		Game.last.showIf = function(){
		...
		CCSE.NewHeavenlyUpgrade("Magical angels","You retain <b>5%</b> of your magic regen while offline.<q>Heaven hath granted thine eternal* magics.<br>(* not that eternal and not that great)</q>",121060821,[19,9,MOD.custIcons],86,-1262,["Chimera"]);
		MOD.ApplyOrder();
		MOD.applyCredit("dpareja");
		MOD.New();
		Game.last.showIf = function(){
		...
		CCSE.NewHeavenlyUpgrade("Bridge","No effect whatsoever.<q>We're getting closer to something here...</q>",1000000000000,[16,11],-30,986,["Legacy"])
		MOD.ApplyOrder();
		MOD.applyCredit("1234abcdcba4321");
		Game.last.showIf=function(){
			var HUsBought = 0;
			for (let i in Game.Upgrades) {
				if (Game.Upgrades[i].pool == 'prestige' && Game.Upgrades[i].bought) {
					HUsBought++
				}
			}
		...
		CCSE.NewHeavenlyUpgrade("Gold suit",'Unlocks the <span style=color:Gold;>gold suit</span>.<q>Tint the world <span style=color:Gold;>GOLD</span>!</q>',10000000000000,[13,7],50,1133,["Bridge"])
		...
		CCSE.NewUpgrade("Gold suit [off]",'Applies a <span style=color:Gold;>gold suit</span> onto everything <small>(besides that, does nothing)</small>!',0,[13,6,])
		Game.last.toggleInto='Gold suit [on]'
		Game.last.pool = 'toggle'
		Game.last.buyFunction = function(){MOD.enableGold()}
		MOD.ApplyOrder();

		CCSE.NewUpgrade("Gold suit [on]",'<b><div style="text-align:center;">Currently on</div></b><div class=\"line\"></div>Applies a <span style=color:Gold;>gold suit</span> onto everything <small>(besides that, does nothing)</small>!',0,[13,7,])
		Game.last.toggleInto='Gold suit [off]'
		Game.last.pool = 'toggle'
		Game.last.buyFunction = function(){MOD.disableGold()}
		MOD.ApplyOrder();

		CCSE.NewHeavenlyUpgrade("Ankh",
		"<b>Doubles</b>:<br>" +
		"CpS, CpC,<br>"+
		"Wrinkler spawn rate, Wrinkler burst rewards, Max wrinklers,<br>" +
		"Golden cookie spawn rate, duration, and power,<br>" +
		"Drop rates, Prestige power, Cookie power,<br>" +
		"Pantheon slots, Magic regeneration.<br>" +
		"<b>Halves</b>:<br>" +
		"Building prices, Upgrade prices,<br>" +
		"Pantheon swap cooldown, Spell cost, Spell backfire rate." +
		"<q>Only for the real gods of cookie clicker.</q>",
		385000000000000000000000000000000000000000000,[8,9,MOD.custIcons],-80,1133,["Gold suit"])
		MOD.ApplyOrder();
		MOD.applyCredit("Baby_Sloth"); //Note, they clarified just CpS, but the idea just stated "Doubles Everything", and so I decided "Why not double more? Given the upgrades we have, 2x and 7x CpS is rather cheap! It *is* unbalanced, but in the Tredecillions!"
		MOD.New();
		Game.last.showIf=function(){
			var HUsBought = 0;
			for (let i in Game.Upgrades) {
			...
			return HUsBought>=170;
		}

		CCSE.NewHeavenlyUpgrade("Trophy", "<br><p style='font-size:40px;'><b>YOU WIN!!!</b></p><br><q>after purchasing this upgrade</q>",1000000000000000000000000000000000000000000000000,[9,9,MOD.custIcons],-30,1282,["Ankh"])
		MOD.ApplyOrder();
		MOD.applyCredit("1234abcdcba4321");
		Game.last.descFunc = function(){
		...
		Game.customBuildStore.push(function(){
			if (Game.mods["percentage graph mod"] || typeof PercentageMod !== "undefined") {
				PercentageMod.insertGraphs();
			}
			if (Game.mods["ratioMod"]) {
				let M = Game.mods["ratioMod"]

				for (var i in Game.ObjectsById) {
					// Callback is on a timeout so it fires after the Game.RefreshStore() function
					l('product'+i).addEventListener('click', function() { setTimeout(M.onClick(M), 60) });
					// Change font to match the cookie counter
					l('product'+i).style.fontFamily = 'Merriweather';
					// Create HTML element to display the ratio
					M.createSpan('productPrice'+i, 'productRatio'+i, M.colors[1]);
				}
				// Create color picker for ratio
				for (var i = 0; i < 3; i++) {
					M.createColor('products', 'storeBulk', 'ratioColor'+i, M.colors[i]);
					// Add event listener for color change
					let j = i;
					l('ratioColor'+i).addEventListener("change", function() { M.colors[j] = this.value, M.onClick(M); });
				}
				// Set position for color pickers
				l('ratioColor0').style.right = '26px';
				l('ratioColor1').style.right = '13px';
			}
		});
		if (Game.mods["percentage graph mod"] || typeof PercentageMod !== "undefined") {
			CCSE.ReplaceCodeIntoFunction(`PercentageMod.updatePercentages`,`var percDiv = obj.l.getElementsByClassName("content")[0].getElementsByClassName("percentage")[0];`,`
				// PercentageMod.updatePercentages injection point 1 by HA;
				if (typeof obj.l.getElementsByClassName("content")[0].getElementsByClassName("percentage")[0]) return;`,-1)
		}
		...
		Game.customAscend.push(function(){
			if (Game.Has('Evil face') && (Game.mods["GabeDFPU"].tooUglyTimesBoughtTOTAL < 1 || Game.mods["GabeDFPU"].tooUglyTimesBought < 1)) {
				if (Game.mods["CookieMonster"]) {
					document.children[0].children[1].children[0].children[1].children[17].children[8].removeChild(Game.mods["GabeDFPU"].evilFace)
				} else {
					document.children[0].children[1].children[0].children[1].children[16].children[8].removeChild(Game.mods["GabeDFPU"].evilFace)
				}
			}
		...
		Game.registerHook('cookiesPerClick',(cpc) => {
			...
			if (Game.Has('Cursor black magic')) {cpcMult += (0.2 * Math.floor(Game.Objects["Cursor"].amount / 100))};
			if (Game.Has('Heavenly mouse')) { //Thank you Knight Breeze for effectively helping me realize that log(0) is -infinity. And as a whole forcing me to force CE to load always, and removing the bug I caused during the slight balance change of this upgrade.
				let bld = Math.max(1, Math.trunc(Game.BuildingsOwned / 10));
				let mult = (Math.log(bld) / 10) + 1;
				cpcMult *= CE.precision(mult,0.1);
			}; //weakened, from Game.BuildingsOwned / 10, to log;
			...
			if (Game.Has("Stacked clicking")) {
				var mult = 1 + (Game.Objects["Cursor"].amount / 1000);
				if (Game.Has("Repetitive clicking")) mult *= mult;
				cpcMult *= mult;
			};

			//(provided mult: (Clicks Per Second / 5) + 1)
			if (Game.Has("Angelic mouse")) {
				cpcMult *= ((MOD.ClicksPerSecond / 5) + 1)
			}
			...
			var finalcpc = (cpc + cpcAdd) * cpcMult

			if (Game.Has("Ankh")) finalcpc*=2;
		...
		Game.registerHook('cps',(cps) => {
			...
			// if (Game.Has('Deep thought') || Game.Has('Lucy in the sky with cookies') || Game.Has('Instrumentability')) {
				...
				for (let i in Game.Upgrades) {
					...
					if (upgrade.pool == 'cookie' && upgrade.bought == 1 && upgrade.power != undefined) {
						if (upgrade.power * 1 == NaN || isNaN(upgrade.power * 1)) {
							//assume that this be a function
							var pow = upgrade.power();
							if (Game.Has("Divine baking I")) cookieadditionalPow+=(pow*0.25);
							if (Game.Has("Divine baking II")) cookieadditionalPow+=(pow*0.25);
							if (Game.Has("Divine baking III")) cookieadditionalPow+=(pow*0.25);
							if (Game.Has("Divine baking IV")) cookieadditionalPow+=(pow*0.25);
							if (Game.Has("Ankh")) cookieadditionalPow+=pow;
						} else {
							if (Game.Has("Divine baking I")) cookieadditionalPow+=(upgrade.power*0.25);
							if (Game.Has("Divine baking II")) cookieadditionalPow+=(upgrade.power*0.25);
							if (Game.Has("Divine baking III")) cookieadditionalPow+=(upgrade.power*0.25);
							if (Game.Has("Divine baking IV")) cookieadditionalPow+=(upgrade.power*0.25);
							if (Game.Has("Ankh")) cookieadditionalPow+=upgrade.power;
						}
					}
				}
				...
				//me = 0
			// }
			cpsMult*=cookieadditionalPow;

			if (Game.Has("...said 4.")) cpsMult*=1.04;
			if (Game.Has("Team force bread")) cpsMult*=1.44;
			
			if (Game.Has('Golden power [off]') && Game.Has('Golden power')) {
					cpsMult*=0.90;
			}

			if (Game.Has('Wrathful building booster [off]')) {
				//we have it
				if (Game.elderWrath > 0) {
					var percent = 0.005;
				
					percent += ((Game.elderWrath - 1) / 100)
				
					var cpsmul = ((Game.BuildingsOwned * percent) + 1)

					cpsMult *= cpsmul
				}
			}

			if (Game.Has("Heavenly building booster [off]")) {
				//we have it as well
				var percent = 0.005;
				percent += (0.001 * Game.BuildingsOwned)
				percent -= (0.0005 * Game.wrinklers.length)
			...
			let baseCPS = cps * cpsMult + evilCPS + cpsAdd;
			let uglyCPS = 0;
			if (MOD.TooUglyTimesBought >= 6) {
				let additionalCPS = baseCPS * 0.066666
				uglyCPS += additionalCPS
			}

			if (MOD.TooUglyTimesBought >= 6666) {
				let additionalCPS = baseCPS * 6.666
				uglyCPS += additionalCPS
			}

			if (MOD.TooUglyTimesBought >= 66666) {
				uglyCPS *= (Math.log10(baseCPS + 10) * 0.066666);
			}
			...
			var finalcps = ((cps + evilCPS + cpsAdd + uglyCPS) *cpsMult) * cpsMultMult //what, what the actual hell is even going on here?
		
			if (Game.Has("Ankh")) finalcps*=2;
		...
		Game.registerHook('reincarnate',() => {
			...
			if (Game.Has('Golden power')) Game.Unlock("Golden power [off]");
			if (Game.Has('Building boosters')) {Game.Unlock("Wrathful building booster [off]"); Game.Unlock("Heavenly building booster [off]")}
			if (Game.Has('Autoclick upgrade')) Game.Unlock("Autoclicker [off]")
		...
		MOD.ClicksPerSecond = 0;

		Game.registerHook('logic',() => {
			if (Game.T % (Game.fps / Game.mods["GabeDFPU"].autoclickerclickspersecond) === 0 && Game.Has("Autoclicker [off]") && Game.ascensionMode != 1) {
		...
		Game.registerHook('check',() => {
			...
			if (Game.mods["GabeDFPU"].tooUglyTimesBought >= 6666666666666666 || Game.mods["GabeDFPU"].tooUglyTimesBoughtTOTAL >= 6666666666666666) {
				Game.Win("Officially bought"); //should be the max...
			}

			Game.mods["GabeDFPU"].MiniGameChecks(); //hope minigames exist, if not, we'll keep running the func
		})

		Game.registerHook('click',()=>{
			MOD.ClicksPerSecond+=1;
			setTimeout(function(){
				MOD.ClicksPerSecond=Math.max(0,MOD.ClicksPerSecond-1);
			},1000)
		});
		...
		Game.customModifyBuildingPrice.push(function(building,price){
			...
			if (Game.Has('Team force bread') && building.name=='Factory') priceMod*=0.94;
			if (Game.Has("Ankh")) priceMod/=2;
			return priceMod;
		})
		...
		Game.customUpgradesAll.getPrice.push(function(upgrade){
			...
			if (Game.Has("Ankh")) price/=2;
			}
			return price;
		})

		if (!cookieMonsterDetected) {
			Game.customMilkMult.push(function(){
				var milkMult = 1;
				if (Game.Has('Santa\'s near infinite milk and cookies')) milkMult*=1.05;
				return milkMult
			});
		}
		...
		Game.customShimmerTypes['golden'].durationMult.push(function(me,choice){ //GC Living Duration
			...
			if (Game.Has("Ankh")) dur*=2;
			return CE.precision(dur,0.001);
		})
		...
		Game.customShimmerTypes['golden'].customEffectDurMod.push(function(me,choice){ //GC Effect Duration
			...
			if (Game.Has('Redoubled essence of redoubled essence of redoubled luck') && Math.random() < 0.07 && Game.goldenClicksLocal >= 77 && (choice == 'frenzy' || choice == 'blood frenzy' || choice == 'click frenzy'))dur*=2; 
		...
		Game.customShimmerTypes['golden'].getTimeMod.push(function(){ //GC Spawnrate
			...
			if (Game.Has("Ankh"))time/=2;
			return CE.precision(time,0.001);
		})
		...
		Game.customShimmerTypes['reindeer'].customReindeerPopMoni.push(function(){
			...
			if (Game.Has("Ankh")) moni*=2;
			return CE.precision(moni,0.1);
		})
		...
		Game.customBuildings['Grandma'].cpsMult.push(function(){
			...
			if (Game.Has('Heavenly grandmas')) cpsMult *= 2;
			return cpsMult
		})

		Game.customBuildings['Factory'].cpsMult.push(function(){
			var cpsMult = 1;
			if (Game.Has('The universe...')) cpsMult*=1.04;
			if (Game.Has('...said 4.')) cpsMult*=1.44;
			if (Game.Has('Team force bread')) cpsMult*=4.44;
			return cpsMult
		})

		Game.customDropRateMult.push(function(){
			rateMult = 1;
			if (Game.Has('Santa\'s heavenly bag')) rateMult *= 1.1;
			if (Game.Has("Ankh")) rateMult *=2;
			return rateMult
		})
		...
		Game.customGetWrinklersMax.push(function(n){
			...
			if (Game.Has("Ankh")) num*=2;
			if (Game.Has("Ankh")) num+(n*2);
			return num;
		})
		...
		Game.customDropEgg.push(function(){
			var failRateMult = 1; 
			if (Game.Has('Permanent omelette')) failRateMult*=0.9;
			if (Game.Has("Ankh")) failRateMult/=2;
			return failRateMult;
		});
		...
		Game.customGoldenBuffPows['clot'].push(function(pow){
			...
			if (Game.Has('Redoubled essence of redoubled essence of redoubled luck') && Math.random() < 0.07 && Game.goldenClicksLocal >= 77) {
				pow/=2; //same thing
			}
			if (Game.Has('Golden power [off]') && Game.Has('Golden power')) {
				pow/=2;
		...
		for (let i in Game.customGoldenBuffPows) {
			if (i != 'chain cookie maxpayout' && i != 'chain cookie randomEndChance' && i != 'clot') Game.customGoldenBuffPows[i].push(function(pow){
				if (Game.Has('Golden power [off]') && Game.Has('Golden power')) {
					pow*=2;
				}
				if (Game.Has('Ankh')) {
		...
		Game.customGoldenBuffPows['multiply cookies'].push(function(moni) {
			...
			return CE.precision(moni,0.001);
		});
		...
		Game.customGetSeasonDuration.push(function(){
			...
			if (Game.Has("Ankh")) durmult*=2;
			return CE.precision(durmult,1);
		});
		...
		Game.customPriceIncrease.push(function(priceIncrease){
			...
			// if (Game.Has("Ankh")) priceIncrease /=2; 
		...
		Game.customHeavenlyMultiplier.push(function(){
			...
			if (Game.Has("Ankh"))mult*=2;
			return mult;
		});

		Game.customChocolateEggBurst.push(function(cookies){
			var origCookies = Game.cookies*0.05;
			var dif = cookies / origCookies;

			if (Game.Has("Richest chocolate")) {
				cookies = Math.max(Game.cookies * 0.125,Game.cookiesPs * 1800);
			} else if (Game.Has("Richer chocolate")) {
				cookies = Math.max(Game.cookies * 0.10,Game.cookiesPs * 1200);
			} else if (Game.Has("Rich chocolate")) {
				cookies = Math.max(Game.cookies * 0.075,Game.cookiesPs * 600);
			}

			cookies *= dif; //attempt to retain original/previous mult(s);

			return cookies;
		});

		Game.customUpgrades["Chocolate egg"].unlock.push(function(){
			if (Game.Has("Strengthened twin") && Game.Has("Dual chocolate")) {
				if (Math.random() < 0.25) Game.Unlock("Bonus chocolate egg")
			} else if (Game.Has("Dual chocolate")){
				if (Math.random() < 0.20) Game.Unlock("Bonus chocolate egg")
			}
		});
		...
		Game.customGoldenSpawned.push(function(me,i){
			if (Game.Has("Ankh")) {
				if (Number(i) == 0) i = 'golden'
				if (Number(i) == 1) i = 'reindeer'
		...
		Game.mods["GabeDFPU"].Changelog = '</div><div class="subsection update small">'+ //credits for bug fixing shall be above the actual changelog always
			'<div class="title">Bug fixes</div>'+
			'<div class="listing">Thought I should credit those that help in fixing bugs, intentionally or not;</div>'+
			'<div class="listing">Credit: <b>Knight Breeze</b> for:</div>'+
			'<div class="listing">&bull; Spotting the need to force CE to load before HA/HR.</div>'+
			'<div class="listing">&bull; Catching a bug in "Heavenly mouse" where log(0) led to -Infinity cookies.</div>'+
			'<div class="listing">Credit: <b>there\'s no i in shrimp</b> for:</div>'+
			'<div class="listing">&bull; Helping me find out compatibility bugs with CookieMonster (unfortunately, the code of the mod is unreadable at least for me, so... don\'t expect compatibility with CookieMonster)</div>'+
			'<div class="listing">&bull; Notifying me of wrinklers being broken (I was two letters off and it broke, that is fixed now)</div>'+
			'<div class="listing">Credit: <b>Deinonycorvus</b> for:</div>'+
			'<div class="listing">&bull; Finding a bug with Divine Baking that appeared to be caused due to some upgrades having their power be functions (therefore causing NaN CpS)</div>'+
			'<div class="listing">&bull; Finding a bug with Too ugly! and the Autoclicker (Too ugly! now has a purchase cap of 6.66 quadrillion, and the Autoclicker has a toggle and has been buffed to 15 clicks a second)</div>'+
			'<div class="listing">Credit: <b>uwuuwuwuwuwuwu</b> for:</div>'+
			'<div class="listing">&bull; Giving me a list of mods that they use, leading to Unbounded being fixed (had to inject into Game.BuildStore just for two mods to work)</div>'+
			'</div><div class="subsection update">'+
			'<div class="title">June 19th, 2025 - The "final" update</div>'+
			'<div class="listing">&bull; fixed a few more bugs.</div>'+
			'<div class="listing">&bull; added a few more heavenly upgrades that for sure can cause bugs due to the amount of code injections that were done.</div>'+
			'<div class="listing">&bull; We\'re at 153 heavenly upgrades, a bit above MHUR\'s 141.</div>'+
			'<div class="listing">&bull; please... I might add in the scrapped upgrades if people want more content...</div>'+
			'<div class="listing">&bull; Made Too ugly! have higher price-scaling.</div>'+
			'<div class="listing">&bull; Heavily rebalanced Too ugly!</div>'+
			'<div class="listing">&bull; Made Too ugly!\'s final shadow achievement (not from the archives) far more easily attainable, and less dependent on if the integer has inprecision.</div>'+
			'<div class="listing">&bull; Buffed the autoclicker upgrade along with making it a toggle (it still won\'t click on the shimmering veil if the veil and autoclicker are both on).</div>'+
			'<div class="listing">&bull; Hopefully there aren\'t many more balance issues or bugs.</div>'+
			'<div class="listing">&bull; Removed "New!" tag until I add more upgrades (more so if).</div>'+
			'<div class="listing">&bull; Finally out of alpha, technically now it\'s finally Heavenly Archives 1.0.</div>'+
			'</div><div class="subsection update small">'+
			'<div class="title">June 17th, 2025 - yet another bug fix.</div>'+
			'<div class="listing">&bull; added a few more heavenly upgrades.</div>'+
			'<div class="listing">&bull; fixed yet another bug.</div>'+
			'<div class="listing">&bull; I still have to get to adding in more upgrades, screw this I\'m going to hard-focus on adding in upgrades.</div>'+
			'<div class="listing">&bull; and fix whatever bugs show up.</div>'+
			//nope //'<div class="listing">&bull; re-did the order of all of every Heavenly Upgrade, rather than being at the far end, they are rather interspersed with the actual in-game Heavenly Upgrades, akin to what I attempted to do for MHUU, but done rather properly this time around.</div>'+
				   //'<div class="listing">&bull; although the many upgrade duplicates (eggs, christmas, halloween and valentines stuffs (probably half the mod\'s content if I\'m being honest here)) do not have their order changed for the sake of it looking neat and being less \'bloaty\'.</div>'+
			'</div><div class="subsection update small">'+
			'<div class="title">June 13th, 2025 - emergency update.</div>'+
			'<div class="listing">&bull; rebalanced "Heavenly mouse"</div>'+
			'<div class="listing">&bull; rebalanced "Too ugly!" (I know I did this at 3:00 AM June 14th, but it still counts, and technically combining updates into one small emergency update)</div>'+
			'<div class="listing">&bull; fixed a broken egg from the previous small update (caused golden cookies to be infinitely clickable (kinda broken))</div>'+
			'<div class="listing">&bull; keeping last update\'s added heavenly upgrades stay tagged as "New!" for now, so consider this an expansion to the previous update, effectively making it one <b>whole upgrade</b>.</div>'+
			'<div class="listing">&bull; also added the credits and fixed some bugs here and there (haven\'t added many Heavenly Upgrades yet... sorry!!)</div>'+
			'</div><div class="subsection update small">'+
			...
			'<div class="listing">&bull; new shy upgrade that is effectively <b>D.E.O.R.L.</b> (Distilled Essence of Redoubled Luck), but more often.</div>'+
	...
	GabeDFPU.save = function () {
		if (!Game.mods["GabeDFPU"].saveTempleSlot) {
			Game.mods["GabeDFPU"].saveTempleSlot = function(id) {
				const temple = Game.Objects["Temple"];
				const mg = temple && temple.minigame;
				return mg ? mg.slot[id] : -1;
			}
		}
		let saveData = {
			...
			cookieClickVal: Game.mods["GabeDFPU"].cookclickval,
			ankhSlotJade: Game.mods["GabeDFPU"].saveTempleSlot(Game.mods["GabeDFPU"].allSlotID["jade"]),
			ankhSlotRuby: Game.mods["GabeDFPU"].saveTempleSlot(Game.mods["GabeDFPU"].allSlotID["ruby"]),
			ankhSlotDiamond: Game.mods["GabeDFPU"].saveTempleSlot(Game.mods["GabeDFPU"].allSlotID["diamond"]),
			ankhSlotJade2: Game.mods["GabeDFPU"].saveTempleSlot(Game.mods["GabeDFPU"].allSlotID["jade2"]),
			addAltarJade: Game.mods["GabeDFPU"].saveTempleSlot(Game.mods["GabeDFPU"].jadeSlotID),
		};
		return JSON.stringify(saveData);
	}
	GabeDFPU.load = function (str) {
		if (str) {
			...
			Game.mods["GabeDFPU"].cookclickval = saveData.cookieClickVal ?? 0;
			Game.mods["GabeDFPU"].allSlot["jade"] = saveData.ankhSlotJade || -1;
			Game.mods["GabeDFPU"].allSlot["ruby"] = saveData.ankhSlotRuby || -1;
	...
	if (CCSE.ConfirmGameVersion(GabeDFPU.name, GabeDFPU.version, GabeDFPU.GameVersion)) Game.registerMod(GabeDFPU.name, GabeDFPU);

};

if (!GabeDFPU.isLoaded) {
	if (CEModLib && CEModLib.isInitialized) {
		GabeDFPU.launch();
	} else {
		if (!CEModLib) var CEModLib = {};
		if (!CEModLib.postLoadHooks) CEModLib.postLoadHooks = [];
		CEModLib.postLoadHooks.push(GabeDFPU.launch);
	}
}