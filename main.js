...
var SoundI = 0;
var SoundIMax = 20;
for (var i = 0; i < SoundIMax; i++) { SoundInsts[i] = new Audio(); }
...
else if (!App && window.location.search.indexOf('beta') > -1) Game.beta = 1;
...
Game.Launch = function () {
	...
	Game.Init = function () {
		...
		Game.DefaultPrefs = function () {
			...
			Game.prefs.crates = 1;//show crates around icons in stats
		...
		Game.bakeryNameSet = function (what) {
			if (Game.bakeryName != what && what == 'harley') {//added by request as thanks to the very nice person who kept my twitter handle safe when my account got hacked by crypto scammers
		...
		TOOLTIP
		=======================================================================================*/
		Game.tooltip = { text: '', lasttext: '', rawtext: '', lastrawtext: '', x: 0, y: 0, origin: '', on: 0, tt: l('tooltip'), tta: l('tooltipAnchor'), shouldHide: 1, dynamic: 0, from: 0, lastfrom: 0 };
		...
		Game.CheckUpdates = function () {
			if (!App && !Mods) getJson(DataDir + 'version.json', function (r) {
		...
		Game.mouseCps = function () {
			...
			add = add * num;
			let OnePercentOfCPS = Game.cookiesPs * 0.01;
			if (Game.Has('Plastic mouse')) add += OnePercentOfCPS;
			if (Game.Has('Iron mouse')) add += OnePercentOfCPS;
			if (Game.Has('Titanium mouse')) add += OnePercentOfCPS;
			if (Game.Has('Adamantium mouse')) add += OnePercentOfCPS;
			if (Game.Has('Unobtainium mouse')) add += OnePercentOfCPS;
			if (Game.Has('Eludium mouse')) add += OnePercentOfCPS;
			if (Game.Has('Wishalloy mouse')) add += OnePercentOfCPS;
			if (Game.Has('Fantasteel mouse')) add += OnePercentOfCPS;
			if (Game.Has('Nevercrack mouse')) add += OnePercentOfCPS;
			if (Game.Has('Armythril mouse')) add += OnePercentOfCPS;
			if (Game.Has('Technobsidian mouse')) add += OnePercentOfCPS;
			if (Game.Has('Plasmarble mouse')) add += OnePercentOfCPS;
			if (Game.Has('Miraculite mouse')) add += OnePercentOfCPS;
			if (Game.Has('Aetherice mouse')) add += OnePercentOfCPS;

			if (Game.Has('Fortune #104')) add += OnePercentOfCPS;
			var mult = 1;
		...
		Game.Notify = function (title, desc, pic, quick, noLog) {
			if (Game.prefs.notifs) {
				quick = quick ? Math.min(6, quick) : 6;
			}
		...
		Game.UpdateMenu = function () {
			...
			if (Game.onMenu == 'prefs') {
				...
				str +=
					...
					(Music ? Game.WriteSlider('volumeMusicSlider', loc("Volume (music)"), '[$]%', function () { return Game.volumeMusic; }, 'Game.setVolumeMusic(Math.round(l(\'volumeMusicSlider\').value));l(\'volumeMusicSliderRightText\').innerHTML=Game.volumeMusic+\'%\';') : '') +
					...
					'<br>' +
					(Music ? Game.WritePrefButton('bgMusic', 'bgMusicButton', loc("Music in background") + ON, loc("Music in background") + OFF, '') + '<label>(' + loc("music will keep playing even when the game window isn't focused") + ')</label><br>' : '') +
					...
					(!(App || Mods && Mods.writeModUI) ? '<div class="listing"><a class="option smallFancyButton" ' + Game.clickStr + '="Game.CheckModData();PlaySound(\'snd/tick.mp3\');">' + loc("Check mod data") + '</a><label>(' + loc("view and delete save data created by mods") + ')</label></div>' : '') +
		...
		Game.Object = function (name, commonName, desc, icon, iconColumn, art, price, cps, buyFunction) {
			...
			this.getPrice = function (n) {
				...
				return Math.min(Math.ceil(price), 2e254);
			}
			...
			this.tooltip = function () {
				...
				return '<div style="position:absolute;left:1px;top:1px;right:1px;bottom:1px;background:linear-gradient(125deg,' + (false ? 'rgba(15,115,130,1) 0%,rgba(15,115,130,0)' : 'rgba(50,40,40,1) 0%,rgba(50,40,40,0)') + ' 20%);mix-blend-mode:screen;z-index:1;"></div><div style="z-index:10;min-width:350px;padding:8px;position:relative;" id="tooltipBuilding"><div class="icon" style="float:left;margin-left:-8px;margin-top:-8px;' + writeIcon(icon) + '"></div><div style="float:right;text-align:right;"><span class="price' + (canBuy ? '' : ' disabled') + '">' + Beautify(Math.round(price)) + '</span>' + Game.costDetails(price) + '</div><div class="name">' + name + '</div>' + '<small><div class="tag">' + loc("owned: %1", me.amount) + '</div>' + (me.free > 0 ? '<div class="tag">' + loc("free: %1!", me.free) + '</div>' : '') + '</small>' +
					...
					(me.totalCookies > 0 ? (
						...
						'<div class="descriptionBlock">' + loc("%1 producing <b>%2</b> per second", [loc("%1 " + me.bsingle, LBeautify(me.amount)), loc("%1 cookie", LBeautify(me.storedTotalModCps * Game.globalCpsMult, 1))]) + ' (' + loc("<b>%1%</b> of total CpS", Beautify(Game.cookiesPs > 0 ? ((me.amount > 0 ? ((me.storedTotalModCps * Game.globalCpsMult) / Game.cookiesPs) : 0) * 100) : 0, 1)) + ')</div>' +
		...
		Game.YouCustomizer.render = function () {
			...
			Game.Loader.waitForLoad([img, imgAddons], function () {
				...
				if (!App && (Game.local)) {
					...
					ctxAddon = canvasAddon.getContext('2d', { willReadFrequently: true });
		...
		Game.DrawWrinklers = function () {
			...
			for (var i in Game.wrinklers) {
				...
				if (me.phase > 0) {
					...
					if (!Game.WINKLERS && Game.prefs.notScary) ctx.drawImage(Pic(Math.sin(T * 0.003 + i * 11 + 137 + Math.sin(T * 0.017 + i * 13)) > 0.9997 ? 'wrinklerBlink.png' : 'wrinklerGooglies.png'), -sw / 2, -10 + 1 * Math.sin(Game.T * 0.2 + i * 3 + 1.2), sw, sh);
	...
	Game.Logic = function () {
		...
		if (!Game.OnAscend && Game.AscendTimer == 0) {
			...
			if (Game.prefs.wobbly) {
				...
				Game.BigCookieRotTarget += 0.5 * (0.125 * Math.sin(Game.T) - Game.BigCookieRotTarget)
			...
			for (var i in Game.Objects) {
				var me = Game.Objects[i];
				me.totalCookies += (me.storedTotalModCps * Game.globalCpsMult) / Game.fps;
			}
			...
			var launchSteps = [
				...
				function (next) {
					if (typeof PRESETMODS !== 'undefined' && PRESETMODS.length > 0) {
						...
						var modError = function () { console.log('error loading a preset mod'); };
						for (var i = 0; i < mods.length; i++) { LoadScript(mods[i], modLoaded, modError); }
					}
					else if (next) next();
				},
				function (next) {
